<%@ Page Title="" Language="vb" AutoEventWireup="false" MasterPageFile="~/Site.Master" CodeBehind="FormularioCitas.aspx.vb" Inherits="ProyectoEventos.FormularioCitas" %>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">    <asp:hiddenfield id="editando" runat="server" />
<asp:DropDownList ID="ddl_doctor" runat="server" />
<asp:DropDownList ID="ddl_paciente" runat="server" />
<asp:TextBox ID="txt_fechaCita" TextMode="DateTimeLocal" runat="server" placeholder="Fecha y hora de la cita" />
<asp:TextBox ID="txt_motivo" runat="server" placeholder="Motivo de la cita" />
...
<asp:Button ID="btnActualizar" runat="server" Text="Actualizar" OnClick="btnActualizar_Click" />
  <asp:Button ID="btn_ir_doctores" runat="server" Text="Ir a Doctores"
    CssClass="btn btn-info" OnClick="btn_ir_doctores_Click" />

<asp:Label ID="lbl_mensaje" runat="server" />

<asp:GridView ID="gvCitas" runat="server" AutoGenerateColumns="False" DataKeyNames="IdCita"
    ...
    <Columns>
        ...
        <asp:BoundField DataField="IdCita" HeaderText="ID" ReadOnly="True" Visible="false" />
...
<asp:SqlDataSource ID="SqlDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:II-46ConnectionString %>" ProviderName="<%$ ConnectionStrings:II-46ConnectionString.ProviderName %>" SelectCommand="SELECT * FROM [Cita]"></asp:SqlDataSource>
</asp:Content>
